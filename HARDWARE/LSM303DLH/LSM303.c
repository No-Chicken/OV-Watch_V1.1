...
unsigned char LSM303DLH_Init()
{
	...
	for(retry = 0;retry < 3;retry ++)
	{
		...
		Delayms(1);
		LSM303_WriteOneReg(LSM303_CTRL_REG1_A, 0x27);		//正常模式，10Hz速度
		Delayms(1);															
	...
	for(retry = 0;retry < 3;retry ++)
	{
		LSM303_WriteOneReg(LSM303_CRA_REG_M, 0x90);		//磁场传感器15Hz,正常测量;开启温度计
		Delayms(1);		
...
float Azimuth_Calculate(int16_t Xa, int16_t Ya, int16_t Za, int16_t Xm, int16_t Ym, int16_t Zm)
{
	...
	Hy      = Ym * cosf(roll) + Xm * sinf(roll) * sinf(pitch) - Zm * cosf(pitch) * sinf(roll);	//航向角
	Hx      = Xm * cosf(pitch) + Zm * sinf(pitch);																					//航向角
