...
{
	uint8_t dat;
	dat = IIC_Read_One_Byte(&SPL_bus, CHIP_ADDRESS<<1,addr);
	return dat;
}

void SPL_WriteOneReg(uint8_t addr, uint8_t dat)
{
	IIC_Write_One_Byte(&SPL_bus, CHIP_ADDRESS<<1,addr,dat);
}
...
int32_t Get_Traw()
{
	uint8_t buff[3];
	int32_t Traw;
	buff[0] = SPL_ReadOneReg(TMP_B0);
	buff[1] = SPL_ReadOneReg(TMP_B1);
	buff[2] = SPL_ReadOneReg(TMP_B2);
	Traw = buff[2];
	Traw = Traw << 8 | buff[1];
	Traw = Traw << 8 | buff[0];
	if(Traw & (1<<23))
...
int32_t Get_Praw()
{
	uint8_t buff[3];
	int32_t Praw;
	buff[0] = SPL_ReadOneReg(PRS_B0);
	buff[1] = SPL_ReadOneReg(PRS_B1);
	buff[2] = SPL_ReadOneReg(PRS_B2);
	Praw = buff[2];
	Praw = Praw << 8 | buff[1];
	Praw = Praw << 8 | buff[0];
	if(Praw & (1<<23))
...
uint8_t SPL_init()
{
	...
	SPL_WriteOneReg(PRS_CFG, 0x03);		// Pressure 8x oversampling

	SPL_WriteOneReg(TMP_CFG, 0x83);		// External Temperature 8x oversampling

	SPL_WriteOneReg(MEAS_CFG, 0x07);	// continuous temp and pressure measurement
	...
	if(SPL_ReadOneReg(PRS_CFG)!=0x03 || SPL_ReadOneReg(CFG_REG)!=0x00)
	{return 1;}//ERRO
	else 
	{return 0;}//SUCCESS
}
